...
class Net(nn.Module):
    ...
    def predict(self, x, embed, input_state, pad=True, lookahead_audio=None):
        mod = 0
        if pad:
            if lookahead_audio is not None and self.lookahead:
                # Align chunk to stft_chunk_size multiples, NO zero lookahead
                x, mod = mod_pad(x, chunk_size=self.stft_chunk_size, pad=(0, 0))
                # Append real lookahead audio instead of zeros
                x = torch.cat([x, lookahead_audio], dim=-1)
            else:
                # Fallback: original zero-padding behavior
